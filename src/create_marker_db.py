...
def load_metadata(dbname, data):
    ...
    for index, row in enumerate(data):
        ...
        if len(terms) == 1:
            ...
            vals = make_rows(terms=terms, common_name=common_name, scientific_name=scientific_name, region=region, curr_id=curr_id)
            curs.executemany('INSERT INTO metadata (scientific_name, common_name, region,status) VALUES (?,?,?,?)', vals)
...
def main():
    ...
    parser.add_argument('--status_table',
                        help="""Tab delimited file specifying fish status(endemic,native etc) in different geographic regions.""",
                        required=False)
    ...
    if status_file:
        ...
        load_metadata(dbname=dbname, data=status_data)

    load_sequence(dbname=dbname, fname=mito_file)
   
    # remove temporary direcory
    os.rmdir(TMPDIR) 
    return
