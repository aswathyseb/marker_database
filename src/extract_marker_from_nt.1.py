...
# The blastcmd command should produce a file of the format
...
# This is how the marker is stored in database.
marker_map = {"coi": "COI",
              "co1": "COI",
              "coxi": "COI",
              "cox1": "COI",
...
def parse_nt_seq_table(fname, taxids, marker_gene):
    ...
    for row in stream:
        ...
        if marker_gene == "ALL" or marker == marker_gene:
            out = "\t".join(row)
...
def main():
    ...
    # Check marker gene
    if marker_gene not in ['COI', 'COII', 'COIII', '12S', '16S', 'CYTB', 'COMPLETE_GENOME', 'ALL']:
        print(
            "Specified marker gene is not allowed. Possible options are COI, COII, COIII, 12S, 16S, Cytb, complete_genome.")
        sys.exit()

    if marker_gene == "COMPLETE_GENOME":
        marker_gene = "complete genome"
    if marker_gene == "CYTB":
        marker_gene = "Cyt b"
